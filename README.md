...
- **v1.3.0** (2026-04-08): LLMlink — LLM 驱动的记忆图探索
  - 移除 MLP 神经网络重排，3层管线：FAISS → 启发式 → LLMlink
  - LLMlink 优先：种子记忆 `[[id]]` 引用 ≥ log₂(n) 时纯 LLM 探索，不足时启发式 BFS 补缺
  - 记忆内嵌 `[[id]]` 交叉引用（行内 + `<!-- refs -->` 尾注），LLM 通过 `follow-link` 自主探索
  - `update-links`：对话后直接编辑记忆图（替代 log-feedback 训练数据收集）
  - `generate-titles.mjs`：批量 LLM 生成记忆标题，用于尾注标签
  - MLP 版本保留在 `v1.2-mlp` 分支
